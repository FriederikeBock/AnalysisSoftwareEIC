...
void resolutionGranularity(
  ...
  TString inputFolderNames[3] = {"Fun4All_G4_FullDetector_DefaultGranHCAL_beampipe","Fun4All_G4_FullDetector_HighGranHCAL_Pions_beampipe","Fun4All_G4_FullDetector_MaxHighGranHCAL_Pions_beampipe"};
  ...
  Int_t useE[nEne][3]                = {{ 0}};
  TH1F* h_pion_fhcal_E[nEne][3] 	    = {NULL};
  TH2F* h2D_pion_fhcal_dEtadPhi[nEne][3] 	    = {NULL};
  TH1F* h_pion_fhcal_dEta[nEne][3] 	    = {NULL};
  TH1F* h_pion_fhcal_dPhi[nEne][3] 	    = {NULL};
  Int_t activeE[3] = {0};
  ...
  for(Int_t iInp=0; iInp<3;iInp++){
    ...
    for(Int_t epart=0; epart<NELEMS(partE); epart++){
      ...
      TChain* t_pion_fhcal_clusters = new TChain("ntp_cluster", "ntp_cluster");
      t_pion_fhcal_clusters->Add(Form("/media/nschmidt/external2/EICsimulationOutputs/%s/10000_%1.1f/G4EICDetector_g4fhcal_eval.root",inputFolderNames[iInp].Data(),partE[epart]));
      ...
      if(t_pion_fhcal_clusters){
        ...
        fill_histogram(h_pion_fhcal_E[epart][iInp], t_pion_fhcal_clusters, partE[epart], 0.3,iInp==0 ? 2 : (iInp==1 ? 3 : 4 ), true, etalowdef, etahighdef);
      ...
      TTree *const t_pion_loop_fhcal_clusters =  (TTree *) (new TFile(Form("/media/nschmidt/external2/EICsimulationOutputs/%s/10000_%1.1f/G4EICDetector_g4fhcal_eval.root",inputFolderNames[iInp].Data(),partE[epart]), "READ"))->Get("ntp_gshower");
      TTree *const t_pion_loop_fecal_clusters =  (TTree *) (new TFile(Form("/media/nschmidt/external2/EICsimulationOutputs/%s/10000_%1.1f/G4EICDetector_g4femc_eval.root",inputFolderNames[iInp].Data(),partE[epart]), "READ"))->Get("ntp_gshower");
      if(t_pion_loop_fhcal_clusters && t_pion_loop_fecal_clusters){
        h_pion_fhcal_dEta[epart][iInp] 	= new TH1F(Form("h_pion_fhcal_dEta_%d",epart), "", 100, -0.5, 0.5);
        h_pion_fhcal_dPhi[epart][iInp] 	= new TH1F(Form("h_pion_fhcal_dPhi_%d",epart), "", 100, -0.5, 0.5);
        h2D_pion_fhcal_dEtadPhi[epart][iInp] 	= new TH2F(Form("h2D_pion_fhcal_dEtadPhi_%d",epart), "", 100, -0.5, 0.5, 100, -0.5, 0.5);

        Float_t measured_energy1,true_eta1,true_phi1,rec_eta1,rec_phi1,nTowers1;
        t_pion_loop_fhcal_clusters->SetBranchAddress("e", &measured_energy1);
        t_pion_loop_fhcal_clusters->SetBranchAddress("geta", &true_eta1);
        t_pion_loop_fhcal_clusters->SetBranchAddress("gphi", &true_phi1);
        t_pion_loop_fhcal_clusters->SetBranchAddress("eta", &rec_eta1);
        t_pion_loop_fhcal_clusters->SetBranchAddress("phi", &rec_phi1);
        t_pion_loop_fhcal_clusters->SetBranchAddress("ntowers", &nTowers1);

        // Int_t nentries = Int_t(t_pion_loop_fhcal_clusters->GetEntries()) > Int_t(t_pion_loop_fecal_clusters->GetEntries()) ? Int_t(t_pion_loop_fecal_clusters->GetEntries()) : Int_t(t_pion_loop_fhcal_clusters->GetEntries());
        Int_t nentries = Int_t(t_pion_loop_fhcal_clusters->GetEntries());
        Float_t lastEvt = 0;
        for (Int_t i = 0; i < nentries; ++i) {
          if (t_pion_loop_fhcal_clusters->LoadTree(i) < 0)
            break;

          t_pion_loop_fhcal_clusters->GetEntry(i);

          if ((rec_eta1 > etalowdef && rec_eta1 < etahighdef)
          && (measured_energy1 > 0.1) // above MIP
          // && (nTowers1>=(iInp==0 ? 2 : (iInp==1 ? 3 : 4 ))) // actual cluster with >1 tower
          ){
            h_pion_fhcal_dEta[epart][iInp]->Fill(rec_eta1-true_eta1);
            h_pion_fhcal_dPhi[epart][iInp]->Fill(rec_phi1-true_phi1);
            h2D_pion_fhcal_dEtadPhi[epart][iInp]->Fill(rec_eta1-true_eta1,rec_phi1-true_phi1);
          }
        }
        h_pion_fhcal_dEta[epart][iInp]->Scale(1 / h_pion_fhcal_dEta[epart][iInp]->GetEntries());
        h_pion_fhcal_dPhi[epart][iInp]->Scale(1 / h_pion_fhcal_dPhi[epart][iInp]->GetEntries());
        h2D_pion_fhcal_dEtadPhi[epart][iInp]->Scale(1 / h2D_pion_fhcal_dEtadPhi[epart][iInp]->GetEntries());
      }
    }
  }
 ...
 TH2F * histodEtadPhiDummy                           = new TH2F("histodEtadPhiDummy","histodEtadPhiDummy",1000, -0.5, 0.5,1000,0, 0.0390);
  SetStyleHistoTH2ForGraphs(histodEtadPhiDummy, "#it{E}_{rec} / #it{E}_{true}","norm. counts", 0.85*textSizeSinglePad,textSizeSinglePad, 0.85*textSizeSinglePad,textSizeSinglePad, 0.9,0.81);
  histodEtadPhiDummy->GetXaxis()->SetNoExponent();
  histodEtadPhiDummy->GetYaxis()->SetNdivisions(505,kTRUE);
  histodEtadPhiDummy->GetXaxis()->SetMoreLogLabels(kTRUE);
  padnum =0;
  for(Int_t epart=0; epart<NELEMS(partE); epart++){
    if(!useE[epart][0]) continue;
  	cPNG->SetRightMargin(0.01);
    ...
    for(Int_t iInp=0; iInp<3;iInp++){
      ...
      DrawGammaSetMarker(h_pion_fhcal_dEta[epart][iInp], markerInput[iInp], 2, colorInput[iInp], colorInput[iInp]);
      h_pion_fhcal_dEta[epart][iInp]->Draw("same");
  ...
  cPNG->Print(Form("%s/HCAL_dEta_to_true.%s", outputDir.Data(), suffix.Data()));
  ...
  for(Int_t iInp=0; iInp<3;iInp++){
    ...
    for(Int_t epart=0; epart<NELEMS(partE); epart++){
      ...
      // TLegend* legendHE_gamma           = GetAndSetLegend2(0.30, 0.95-(5*textSizeLabelsRel), 0.95, 0.95,0.7*textSizeLabelsPixel, 1, "", 43, 0.15);
        ...
        // DrawGammaSetMarker(h_pion_fhcal_dEta[epart][iInp], markerInput[iInp], 2, colorInput[iInp], colorInput[iInp]);
        SetStyleHistoTH2ForGraphs(h2D_pion_fhcal_dEtadPhi[epart][iInp], "#it{E}_{rec} / #it{E}_{true}","norm. counts", 0.85*textSizeSinglePad,textSizeSinglePad, 0.85*textSizeSinglePad,textSizeSinglePad, 0.9,0.81);
        h2D_pion_fhcal_dEtadPhi[epart][iInp]->Draw("col");
      ...
      // legendHE_gamma->Draw();
     cPNG->Print(Form("%s/HCAL_dEtadPhi_%s.%s", outputDir.Data(),inputFolderNames[iInp].Data(), suffix.Data()));
  } 
