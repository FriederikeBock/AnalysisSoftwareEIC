...
void resolutionhistos(){
  ...
  Double_t paramsECALcalib[5]= {3.15612e+00,1.57658e-01,1.66441e+00,-2.50979e+02,3.81511e+02}; //noshift
  TF1* func_FEMC_calib   = new TF1("func_FEMC_calib",   "( [0] + [1] * TMath::Log(x) ) / ( 1 + ( [2] * TMath::Exp( ( x - [3] ) / [4] ) ) )",   1, 120);
  func_FEMC_calib->SetParameters(paramsECALcalib);
        ...
        if((clusters_RH_trueID[iclus])==_mcpart_ID[imc]){
          ...
          for(Int_t iclus2=0; iclus2<nclusters_comb_RH; iclus2++){
            ...
            if(clusters_RH_trueID[iclus]==clusters_comb_RH_trueID[iclus]){
              clusterenergy += (clusters_comb_RH_E[iclus]/func_FEMC_calib->Eval(clusters_comb_RH_E[iclus]));
            }
          }
          // if(verbosityRH>1) cout << "\tfound MC:  particle " << imc << "\twith E = " << _mcpart_E[imc] << " GeV" << endl;
