  ...
  for(int icalo=0;icalo<_active_calo;icalo++){
    ...
    for(int ialgo=0;ialgo<_active_algo;ialgo++){
      ...
      for(Int_t iclus=0; iclus<_nclusters_TM; iclus++){
        ...
        h_TMstudies_2D_clus[icalo][ialgo]->Fill(isFwd ? clusters_TM_X[iclus] : clusters_TM_Eta[iclus],isFwd ? clusters_TM_Y[iclus] : clusters_TM_Phi[iclus]);
        for(Int_t iproj=0; iproj<_nProjections; iproj++){
          ...
          if(fillcurrentlargest){
            delta_1_lgst = isFwd ? _track_Proj_x[iproj]-clusters_TM_X[largestClusterID] : projeta-clusters_TM_Eta[largestClusterID];
            delta_2_lgst = isFwd ? _track_Proj_y[iproj]-clusters_TM_Y[largestClusterID] : projphi-clusters_TM_Phi[largestClusterID];
            h_TMstudies_2D_delta_projection[1][icalo][ialgo]->Fill(delta_1_lgst,delta_2_lgst);
            h_TMstudies_dx_vsClsEta_projection[1][icalo][ialgo]->Fill(delta_1_lgst,clusters_TM_Eta[largestClusterID]);
            h_TMstudies_dx_vsTrkEta_projection[1][icalo][ialgo]->Fill(delta_1_lgst,projeta);
          }
          // if(TMath::Sqrt(TMath::Power(delta_1,2)+TMath::Power(_track_Proj_y[iproj]-clusters_TM_Y[iclus],2)) <15)matchfound=true;
          // h_TMstudies_2D_delta_projection[0][icalo][ialgo]->Fill(projeta-clusters_TM_Eta[iclus],projphi-clusters_TM_Phi[iclus]);
        ...
        for(Int_t itrk=0; itrk<_nTracks; itrk++){
          ...
          if(clusters_TM_X[iclus]==0 && clusters_TM_Y[iclus]==0) continue;
          float delta_1 = isFwd ? trackvec.X()-clusters_TM_X[iclus] : tracketa-clusters_TM_Eta[iclus];
          float delta_2 = isFwd ? trackvec.Y()-clusters_TM_Y[iclus] : trackphi-clusters_TM_Phi[iclus];
          h_TMstudies_2D_delta_track[0][icalo][ialgo]->Fill(delta_1,delta_2);
          h_TMstudies_dx_vsClsEta_track[0][icalo][ialgo]->Fill(delta_1,clusters_TM_Eta[iclus]);
          h_TMstudies_dx_vsTrkEta_track[0][icalo][ialgo]->Fill(delta_1,tracketa);
          float delta_1_lgst = 1000;
          float delta_2_lgst = 1000;
          if(fillcurrentlargest){
            delta_1_lgst = isFwd ? trackvec.X()-clusters_TM_X[largestClusterID] : tracketa-clusters_TM_Eta[largestClusterID];
            delta_2_lgst = isFwd ? trackvec.Y()-clusters_TM_Y[largestClusterID] : trackphi-clusters_TM_Phi[largestClusterID];
            h_TMstudies_2D_delta_track[1][icalo][ialgo]->Fill(delta_1_lgst,delta_2_lgst);
            h_TMstudies_dx_vsClsEta_track[1][icalo][ialgo]->Fill(delta_1_lgst,clusters_TM_Eta[largestClusterID]);
        ...
        for(Int_t ihit=0; ihit<_nHitsLayers; ihit++){
          ...
          if( (icalo==kFHCAL && _hits_layerID[ihit]==2) ||
            ...
            if(clusters_TM_X[iclus]==0 && clusters_TM_Y[iclus]==0) continue;
            h_TMstudies_2D_delta_TTLhits[0][icalo][ialgo]->Fill(_hits_x[ihit]-clusters_TM_X[iclus],_hits_y[ihit]-clusters_TM_Y[iclus]);
            // check eta difference
            // TVector3 trackvec(_track_px[itrk],_track_py[itrk],_track_pz[itrk]);
            ...
            // if(fillcurrentlargest){
            //   h_TMstudies_2D_delta_track[1][icalo][ialgo]->Fill(trackvec.X()-clusters_TM_X[largestClusterID],trackvec.Y()-clusters_TM_Y[largestClusterID]);
            ...
            // if(_mcpart_PDG[clusters_TM_trueID[iclus]]==_mcpart_PDG[_clusters_MA_FEMC_trueID[iclus2]] && _mcpart_PDG[clusters_TM_trueID[iclus]]==211){
              if(abs(_clusters_MA_FEMC_X[iclus2]-clusters_TM_X[iclus]) < 15){
                if(abs(_clusters_MA_FEMC_Y[iclus2]-clusters_TM_Y[iclus]) < 15){
                  ...
                  h_TMstudies_2D_delta_calomatch[0][icalo][ialgo]->Fill(_clusters_MA_FEMC_X[iclus2]-clusters_TM_X[iclus],_clusters_MA_FEMC_Y[iclus2]-clusters_TM_Y[iclus]);
