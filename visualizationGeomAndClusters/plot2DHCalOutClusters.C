...
void plot2DHCalOutClusters(
  ...
  float aggE        = 0.1;
  int verbosity     = 1;
  TString labelDet  = Form("HCALout%s, event %d", addName.Data(), plotEvent);
  gROOT->Reset();
  ...
  TString outputDir                             = Form("plots/HCalOut%s_%s",dateForOutput.Data(),collisionsSys.Data());
  ...
  int granularity = 1;
  TH2F* h_IEtaIPhiMapEvt_HCALOUT                   = NULL;
  TH2F* h_IEtaIPhiMapEvt_HCALOUT_Ecut              = NULL;
  TH2F* h_IEtaIPhiMapEvt_HCALOUT_Cl[50]            = {NULL};
    
  // read file
  TChain* t_towers_HCALOUT = new TChain("event_tree", "event_tree");
  t_towers_HCALOUT->Add(nameFile.Data());
  
  const int _maxNTowersDR = towersx * towersy;
  int _nTowers_HCALOUT;
  float* _tower_HCALOUT_E           = new float[_maxNTowersDR];
  int* _tower_HCALOUT_iEta         = new int[_maxNTowersDR];
  int* _tower_HCALOUT_iPhi         = new int[_maxNTowersDR];
  int* _tower_HCALOUT_trueID       = new int[_maxNTowersDR];
  ...
  float sumedE                    = 0;
  if(t_towers_HCALOUT){
    h_IEtaIPhiMapEvt_HCALOUT       = new TH2F("h_IEtaIPhiMapEvt_HCALOUT", "",  towersx+1, -0.5, towersx+0.5, towersy+1, -0.5, towersy+0.5);
    h_IEtaIPhiMapEvt_HCALOUT->Sumw2();
    h_IEtaIPhiMapEvt_HCALOUT_Ecut  = new TH2F("h_IEtaIPhiMapEvt_HCALOUT_Ecut", "",  towersx+1, -0.5, towersx+0.5, towersy+1, -0.5, towersy+0.5);
    h_IEtaIPhiMapEvt_HCALOUT_Ecut->Sumw2();
    for (int i = 0; i < 50; i++){
      h_IEtaIPhiMapEvt_HCALOUT_Cl[i]  = new TH2F(Form("h_IEtaIPhiMapEvt_HCALOUT_Cl%d",i), "",  towersx+1, -0.5, towersx+0.5, towersy+1, -0.5, towersy+0.5);
      h_IEtaIPhiMapEvt_HCALOUT_Cl[i]->Sumw2();
    }
    t_towers_HCALOUT->SetBranchAddress("tower_HCALOUT_N",             &_nTowers_HCALOUT);
    t_towers_HCALOUT->SetBranchAddress("tower_HCALOUT_E",             _tower_HCALOUT_E);
    t_towers_HCALOUT->SetBranchAddress("tower_HCALOUT_iEta",          _tower_HCALOUT_iEta);
    t_towers_HCALOUT->SetBranchAddress("tower_HCALOUT_iPhi",          _tower_HCALOUT_iPhi);
    t_towers_HCALOUT->SetBranchAddress("nMCPart",       &_nMCPart);
    t_towers_HCALOUT->SetBranchAddress("mcpart_px",     _mcpart_px);
    t_towers_HCALOUT->SetBranchAddress("mcpart_py",     _mcpart_py);
    t_towers_HCALOUT->SetBranchAddress("mcpart_pz",     _mcpart_pz);
    t_towers_HCALOUT->SetBranchAddress("mcpart_E",      _mcpart_E);
    
    Int_t nentries = Int_t(t_towers_HCALOUT->GetEntries());
    for (Int_t i = 0; i < nentries; ++i) {
      
      if (t_towers_HCALOUT->LoadTree(i) < 0)
        break;
      if(i!=plotEvent) continue;
      t_towers_HCALOUT->GetEntry(i);
      ...
      for(int itwr=0;(int)itwr<_nTowers_HCALOUT;itwr++){
        sumedE += _tower_HCALOUT_E[itwr];
        if (_tower_HCALOUT_E[itwr] > 1e-3 && verbosity) cout << _tower_HCALOUT_iEta[itwr] << "\t" << _tower_HCALOUT_iPhi[itwr] << "\t"<< _tower_HCALOUT_E[itwr] << endl;
        h_IEtaIPhiMapEvt_HCALOUT->Fill(_tower_HCALOUT_iEta[itwr],_tower_HCALOUT_iPhi[itwr],_tower_HCALOUT_E[itwr]);
        if(_tower_HCALOUT_E[itwr]>0.001){
          h_IEtaIPhiMapEvt_HCALOUT_Ecut->Fill(_tower_HCALOUT_iEta[itwr],_tower_HCALOUT_iPhi[itwr],_tower_HCALOUT_E[itwr]);
        }
      }
    }
  }
  ...
  for(int itow=0; itow<(int)_nTowers_HCALOUT; itow++){
...
//             if(input_towers.at(ait).tower_E >= (cluster_towers.at(tit).tower_E + (aggE/10.))) continue;
      ...
      for (int tic = 0; tic < (int)cluster_towers.size(); tic++){
        ...
        h_IEtaIPhiMapEvt_HCALOUT_Cl[nclusters]->Fill(iEtaTwr, iPhiTwr, eTwr);
      }
  ...
  // 1D PLOT
  SetStyleHistoTH2ForGraphs(h_IEtaIPhiMapEvt_HCALOUT_Ecut, "tower #eta index", "tower #phi index", 0.85*textSizeSinglePad,textSizeSinglePad, 0.85*textSizeSinglePad,textSizeSinglePad, 0.45,1.05);
  h_IEtaIPhiMapEvt_HCALOUT_Ecut->GetXaxis()->SetLabelOffset(-0.015);
  h_IEtaIPhiMapEvt_HCALOUT_Ecut->GetXaxis()->SetTickLength(0.02);
  
    h_IEtaIPhiMapEvt_HCALOUT_Ecut->Draw("colz");
  ...
  //************************************************************************************  
  SetStyleHistoTH2ForGraphs(h_IEtaIPhiMapEvt_HCALOUT_Ecut, "tower #eta index", "tower #phi index", 0.85*textSizeSinglePad,textSizeSinglePad, 0.85*textSizeSinglePad,textSizeSinglePad, 0.4,1.0);
  
    h_IEtaIPhiMapEvt_HCALOUT_Ecut->SetLineColor(kGray+2);
    h_IEtaIPhiMapEvt_HCALOUT_Ecut->Draw("box");
  ...
  //************************************************************************************
    for (int nCl = 0; nCl < nclusters; nCl++){
      h_IEtaIPhiMapEvt_HCALOUT_Cl[nCl]->SetLineColor(colorCluster[nCl]);
      h_IEtaIPhiMapEvt_HCALOUT_Cl[nCl]->Draw("box,same");
    }
    if (collisionsSys.Contains("Single")){
