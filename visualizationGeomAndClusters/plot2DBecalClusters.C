...
void plot2DBecalClusters(
  ...
  float aggE        = 0.010;
  int verbosity     = 1;
  TString labelDet  = Form("BECAL%s, event %d", addName.Data(), plotEvent);
  gROOT->Reset();
  ...
  int granularity = 1;
  TH2F* h_IEtaIPhiMapEvt_BECAL                   = NULL;
  TH2F* h_IEtaIPhiMapEvt_BECAL_Ecut              = NULL;
  TH2F* h_IEtaIPhiMapEvt_BECAL_Cl[50]            = {NULL};
    
  // read file
  TChain* t_towers_BECAL = new TChain("event_tree", "event_tree");
  t_towers_BECAL->Add(nameFile.Data());
  ...
  float sumedE                    = 0;
  if(t_towers_BECAL){
    h_IEtaIPhiMapEvt_BECAL       = new TH2F("h_IEtaIPhiMapEvt_BECAL", "",  towersx+1, -0.5, towersx+0.5, towersy+1, -0.5, towersy+0.5);
    h_IEtaIPhiMapEvt_BECAL->Sumw2();
    h_IEtaIPhiMapEvt_BECAL_Ecut  = new TH2F("h_IEtaIPhiMapEvt_BECAL_Ecut", "",  towersx+1, -0.5, towersx+0.5, towersy+1, -0.5, towersy+0.5);
    h_IEtaIPhiMapEvt_BECAL_Ecut->Sumw2();
    for (int i = 0; i < 50; i++){
      h_IEtaIPhiMapEvt_BECAL_Cl[i]  = new TH2F(Form("h_IEtaIPhiMapEvt_BECAL_Cl%d",i), "",  towersx+1, -0.5, towersx+0.5, towersy+1, -0.5, towersy+0.5);
      h_IEtaIPhiMapEvt_BECAL_Cl[i]->Sumw2();
    }
    t_towers_BECAL->SetBranchAddress("tower_BECAL_N",             &_nTowers_BECAL);
    t_towers_BECAL->SetBranchAddress("tower_BECAL_E",             _tower_BECAL_E);
    t_towers_BECAL->SetBranchAddress("tower_BECAL_iEta",          _tower_BECAL_iEta);
    t_towers_BECAL->SetBranchAddress("tower_BECAL_iPhi",          _tower_BECAL_iPhi);
    t_towers_BECAL->SetBranchAddress("nMCPart",       &_nMCPart);
    t_towers_BECAL->SetBranchAddress("mcpart_px",     _mcpart_px);
    t_towers_BECAL->SetBranchAddress("mcpart_py",     _mcpart_py);
    t_towers_BECAL->SetBranchAddress("mcpart_pz",     _mcpart_pz);
    t_towers_BECAL->SetBranchAddress("mcpart_E",      _mcpart_E);
    
    Int_t nentries = Int_t(t_towers_BECAL->GetEntries());
    for (Int_t i = 0; i < nentries; ++i) {
      
      if (t_towers_BECAL->LoadTree(i) < 0)
        break;
      if(i!=plotEvent) continue;
      t_towers_BECAL->GetEntry(i);
      ...
      for(int itwr=0;(int)itwr<_nTowers_BECAL;itwr++){
        sumedE += _tower_BECAL_E[itwr];
        if (_tower_BECAL_E[itwr] > 1e-3 && verbosity) cout << _tower_BECAL_iEta[itwr] << "\t" << _tower_BECAL_iPhi[itwr] << "\t"<< _tower_BECAL_E[itwr] << endl;
        h_IEtaIPhiMapEvt_BECAL->Fill(_tower_BECAL_iEta[itwr],_tower_BECAL_iPhi[itwr],_tower_BECAL_E[itwr]);
        if(_tower_BECAL_E[itwr]>0.001){
          h_IEtaIPhiMapEvt_BECAL_Ecut->Fill(_tower_BECAL_iEta[itwr],_tower_BECAL_iPhi[itwr],_tower_BECAL_E[itwr]);
        }
      }
    }
  }
  ...
  for(int itow=0; itow<(int)_nTowers_BECAL; itow++){
...
//             if(input_towers.at(ait).tower_E >= (cluster_towers.at(tit).tower_E + (aggE/10.))) continue;
      ...
      for (int tic = 0; tic < (int)cluster_towers.size(); tic++){
        ...
        h_IEtaIPhiMapEvt_BECAL_Cl[nclusters]->Fill(iEtaTwr, iPhiTwr, eTwr);
      }
  ...
  Int_t pixelX  = 35*4.9+fracX*pixelBaseCanvas;
  Int_t pixelY  = 35*2+fracY*pixelBaseCanvas;
  Float_t marginBottom = (Float_t)(35*1.9)/(Float_t)pixelY;
  Float_t marginTop = (Float_t)(35*0.1)/(Float_t)pixelY;
  Float_t marginLeft = 35*2.2/(Float_t)pixelX;
  Float_t marginRight = 35*2.7/(Float_t)pixelX;
  ...
  // 1D PLOT
  SetStyleHistoTH2ForGraphs(h_IEtaIPhiMapEvt_BECAL_Ecut, "tower #eta index", "tower #phi index", 0.85*textSizeSinglePad,textSizeSinglePad, 0.85*textSizeSinglePad,textSizeSinglePad, 0.58,1.05);
  h_IEtaIPhiMapEvt_BECAL_Ecut->GetXaxis()->SetLabelOffset(-0.01);
  h_IEtaIPhiMapEvt_BECAL_Ecut->GetXaxis()->SetTickLength(0.02);
  
    h_IEtaIPhiMapEvt_BECAL_Ecut->Draw("colz");
  ...
  //************************************************************************************  
    h_IEtaIPhiMapEvt_BECAL_Ecut->SetLineColor(kGray+2);
    h_IEtaIPhiMapEvt_BECAL_Ecut->Draw("box");
  ...
  //************************************************************************************
    for (int nCl = 0; nCl < nclusters; nCl++){
      h_IEtaIPhiMapEvt_BECAL_Cl[nCl]->SetLineColor(colorCluster[nCl]);
      h_IEtaIPhiMapEvt_BECAL_Cl[nCl]->Draw("box,same");
    }
    if (collisionsSys.Contains("Single")){
