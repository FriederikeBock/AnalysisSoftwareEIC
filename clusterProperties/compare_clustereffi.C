...
//             cout << Form("ClusterEfficiencyProjections_%s/effi%s_E_%d_%s", calo.Data(), partName[pid].Data(), iEta, nameClus[iCl].Data()) << endl;
      ...
      h_cluster_NClMean_E[iSet][iCl]    = (TH1D*)inputFiles[iSet]->Get(Form("ClusterEfficiencyProjections_%s/h_CS_NClMean_%s_E", calo.Data(), nameClus[iCl].Data()));
      h_cluster_NClMean_MCE[iSet][iCl]  = (TH1D*)inputFiles[iSet]->Get(Form("ClusterEfficiencyProjections_%s/h_CS_NClMean_%s_MCE", calo.Data(), nameClus[iCl].Data()));
    }
  }
    ...
    for (Int_t iCl = 0; iCl < nClus; iCl++){
      for (Int_t pid = 1; pid < nPID; pid++){
        histoDummyEffiE->Draw();
          ...
          drawLatexAdd(Form("%s, %s clusterizer", calo.Data(), nameClus[iCl].Data()),0.14, 0.91-(nLinesCol+1)*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
        ...
        histoDummyEffiMCE->Draw();
          ...
          drawLatexAdd(Form("%s in %1.1f<#eta<%1.1f", partLabel[pid].Data(), etaMin,etaMax),0.14, 0.91-nLinesCol*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
          drawLatexAdd(Form("%s, %s clusterizer", calo.Data(), nameClus[iCl].Data()),0.14, 0.91-(nLinesCol+1)*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
        cReso->Print(Form("%s/%s%s/%s_ClusterEfficiency_MCE_%d_%d.%s", outputDir.Data(), calo.Data(), nameClus[iCl].Data(), partName[pid].Data(), (Int_t)(etaMin*10), (Int_t)(etaMax*10), suffix.Data()));
        ...
        histoDummyEffiE->Draw();
          ...
          if (pTHard.CompareTo("") != 0) drawLatexAdd(pTHard,0.14, 0.91-textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);    
          drawLatexAdd(Form("%s in %1.1f<#eta<%1.1f", partLabel[pid].Data(), etaMin,etaMax),0.14, 0.91-nLinesCol*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
          drawLatexAdd(Form("%s, %s clusterizer, single entry", calo.Data(), nameClus[iCl].Data()),0.14, 0.91-(nLinesCol+1)*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
        ...
        histoDummyEffiMCE->Draw();
          ...
          drawLatexAdd(Form("%s in %1.1f<#eta<%1.1f", partLabel[pid].Data(), etaMin,etaMax),0.14, 0.91-nLinesCol*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
          drawLatexAdd(Form("%s, %s clusterizer, single entry", calo.Data(), nameClus[iCl].Data()),0.14, 0.91-(nLinesCol+1)*textSizeLabelsRel*1.1,textSizeLabelsRel,kFALSE,kFALSE,kFALSE);
        cReso->Print(Form("%s/%s%s/%s_ClusterEfficiencySE_MCE_%d_%d.%s", outputDir.Data(), calo.Data(), nameClus[iCl].Data(), partName[pid].Data(), (Int_t)(etaMin*10), (Int_t)(etaMax*10), suffix.Data()));
  ...
  TLegend* legendNTowerCl   = GetAndSetLegend2(0.14,  0.94-(nSets*textSizeLabelsRel), 0.35,  0.94, textSizeLabelsPixel, 1, "", 43, 0.25);

  for (Int_t iCl = 0; iCl < nClus; iCl++){
//     cout << "clusterizer: " << iCl << "\ta)"<< endl;
    if (calo.CompareTo("FHCAL") == 0) histoDummyNTowerMean->GetYaxis()->SetRangeUser(0, 30);
    histoDummyNTowerMean->Draw();
    legendNTowerCl->Clear();
    for (Int_t iSet = 0; iSet < nSets; iSet++){
//       cout << "set " << iSet << endl;
      ...
      legendNTowerCl->AddEntry(h_cluster_NTowerMean_E[iSet][iCl],labels[iSet].Data(),"p");
    }
    legendNTowerCl->Draw();
    ...
    drawLatexAdd(Form("%s, %s clusterizer, single entry", calo.Data(), nameClus[iCl].Data()),0.95, 0.91-(nLinesCol)*textSizeLabelsRel*1.1,0.85*textSizeLabelsRel,kFALSE,kFALSE,kTRUE);
    cReso->Print(Form("%s/%s%s/NTowerInCluster_Mean_E.%s", outputDir.Data(), calo.Data(), nameClus[iCl].Data(), suffix.Data()));

//     cout << "clusterizer: " << iCl << "\tb)"<< endl;    
